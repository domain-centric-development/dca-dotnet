...
`DomainCentric.BuildingBlocks` keeps the **last released** version in its `<Version>` until it actually
changes: the ArchRules packages take their BuildingBlocks dependency from that element, and the
`archrules` preflight checks that this version is on NuGet.org. Bump it only for a BuildingBlocks release
(same as `buildingBlocksVersion` in the Java build). So a release is one commit plus one tag:
