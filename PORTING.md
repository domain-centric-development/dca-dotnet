...
|---|---|
| `DcaLayout.forBasePackage(p)`; `basePackage()`, `domainSubpackage()`, `apiSubpackage()`, … | `DcaLayout.ForRootNamespace(ns)`; `RootNamespace`, `DomainSegment`, `ApplicationSegment`, `AdapterSegment`, `IncomingSegment`, `OutgoingSegment`, `InfrastructureSegment`, `SharedKernelSegment`, `ApiSegment`, `EventsSegment`, `PublishedSegments`, `UseCaseSuffix`, `RestControllerSuffix`, `ThirdPartyNamespacesAllowedInDomain`, `FrameworkTypes` |
...
| `arch.rootContextPackage(pkg)` / `contextName(pkg)` | `arch.RootContextNamespace(ns)` (nearest declared ancestor, any depth) / `arch.ContextName(ns)` (namespace relative to the root); `SimpleContextName` is obsolete |
| `arch.boundedContextPatterns()` / `…Excluding(p)` | `arch.BoundedContextPatterns()` / `arch.BoundedContextPatternsExcluding(ns)` |
| `arch.contextDomainPatterns()` (declared contexts) / `arch.allDomainPatterns()` (module roots) etc. | same names: `Context{Domain,DomainModel,Application,Adapter}Patterns()` over declared contexts; `All{Domain,DomainModel,Application,SharedOutputPort,Adapter,IncomingAdapter,OutgoingAdapter}Patterns()` over `ModuleRoots()`. Combine with `DcaLayout.AnyOf(...)` — `ResideInNamespaceMatching` takes one regex |
| `arch.moduleRoots()` / `isolatedModuleRoots()` / `moduleRootPatternsExcluding(r)` / `publishedPackagePatternsExcluding(r)` | `arch.ModuleRoots()` / `IsolatedModuleRoots()` / `ModuleRootPatternsExcluding(r)` / `PublishedPatternsExcluding(r)` |
